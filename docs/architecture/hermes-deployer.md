...
- **Новый флоу воркера (`workers/deploy.processor.ts`):**
  ...
  4. расшифровать секреты (`SecretsService` в воркере), отрендерить `env`/`config.yaml`/`compose` (те же чистые функции `provisioning/hermes-config.ts`) → `ProvisioningService.createDockerProject(vmId, "hermes-<deployId>", composeYaml, envContent)`. `environment` = содержимое project-`.env` (секреты); compose секретов не содержит: `config.yaml` едет inline compose-`configs` (`content:` → target `/opt/data/config.yaml`; template 1121 несёт Compose v5.1.4 — inline configs поддерживаются, проверено 2026-07-05), env уходит в контейнер через `env_file: .env`. **Чекпойнт 2026-07-05:** реальный деплой вскрыл, что `deploy.resources.limits.cpus: "2.0"` не даёт контейнеру стартовать на KVM 1 (1 vCPU) — Docker: `range of CPUs is from 0.01 to 1.00`; лимит опущен до `1.0` (`HERMES_CPU_LIMIT`). Также при сбое воркер больше не удаляет **adopted** (подхваченную оплаченную) машину — оставляет для повтора через self-heal;
...
## 21. Real-deploy checkpoint findings + My Agents cabinet (2026-07-05)

Реальный прогон на боевом Hostinger-токене (`DRY_RUN=false`) прошёл end-to-end; по пути вскрылись и закрыты факты, которые нельзя было увидеть на моках:

- **CPU-лимит.** `deploy.resources.limits.cpus: "2.0"` не давал контейнеру стартовать на KVM 1 (1 vCPU): `docker compose up` «успех», но 0 контейнеров, Docker: `range of CPUs is from 0.01 to 1.00`. Опущено до `HERMES_CPU_LIMIT = "1.0"`. Compose на template 1121 — v5.1.4 (inline `configs` поддерживаются). `TELEGRAM_ALLOWED_USERS` (= `deploy.user_id` из TMA-`initData`) и `TELEGRAM_BOT_TOKEN` рендерятся в `.env` — агент отвечает клиенту с первого сообщения без ручного `chat_id`.
- **Гонка оплаты Hostinger (402 / деклайн карты).** `purchaseVM` может вернуть 402/422, а заказ — довершиться асинхронно (~60с), оставив оплаченную VM. Воркер `acquireVm`: (1) **до покупки** ищет adoptable-orphan через `findAdoptableOrphan` — KVM 1 в data-center 11, состояние initial/creating/running, младше 24ч, не занятую **активным** деплоем (`ACTIVE_DEPLOY_STATUSES = pending|creating|configuring|ready`; строка, занятая только failed/deleted, снова свободна); (2) при 402 — `waitForLateVm` (poll listVMs ~3 мин). KVM 1 + dc 11 отсекают control-plane (KVM 2). При сбое deploy'а **adopted-VM не удаляется** (только purchased) — чтобы retry её переиспользовал.
...
- `ProvisioningService.updateDockerProject` = re-push через `createNewProjectV1` (overwrite тем же `project_name`, новый `environment`); `updateProjectV1` без body непригоден для смены env.
- Frontend: экраны `agents` (список карточек: username, цветной бейдж `STATUS_BADGE_CLASS`, провайдер, IP, дата) и `agent` (restart / смена ключа / удаление с confirm). Метка `ready` = «Работает».

**Прод-инфра-заметки:** MCP `hostinger-api` подключён (обёртка над тем же API; docker-logs endpoint отдаёт «project not found» при 0 контейнеров — для диагностики нужен shell на VM). Прод-Postgres чистится через терминал контейнера в Dokploy (внешний :5432 — это Postgres самого Dokploy, не наш).

## 22. Nous Portal (подписка Nous Research) — исследование (2026-07-05)

**Вопрос:** заменить BYOK-ключи бандлом «VPS + подписка Nous, цена = себестоимость + 40%». **Ответ: полностью автоматически — нельзя.** Sourced: portal.nousresearch.com (`/manage-subscription`, `/api-docs`, `/info`, `/terms`), репо hermes-agent (`website/docs/integrations/nous-portal.md`, `guides/oauth-over-ssh.md`, `user-guide/docker.md`, исходники `hermes_cli/auth.py`, `hermes_cli/runtime_provider.py`, issues #12275/#24814).

- **Тарифы Nous Portal** (единая подписка «инференс + инструменты», запущена 2026-04-27): Plus **$20**/мес ($22 кредитов, rollover-кап $10, 400 RPM / 4M TPM); Super **$100** ($110, кап $50, 800 RPM / 8M TPM); Ultra **$200** ($220, кап $100, 1600 RPM / 16M TPM). 300+ моделей (прокси через OpenRouter, слаги вида `anthropic/claude-sonnet-4.6`) + hosted Tool Gateway (web search/scrape, image gen, TTS, browser automation, sandbox). Собственные Hermes-4-70B/405B — со скидкой, но доки **не** рекомендуют их для агентского tool-calling-цикла.
- **Три блокера бандла:** (1) **нет API** покупки подписки / выпуска ключей (issue #24814 закрыт P3; регистрация — только веб-форма за Vercel WAF, автоматизация запрещена ToS §10.2); (2) **ToS §10.2** запрещает resale/предоставление доступа третьим лицам без письменного согласия Nous; §5.1 — один аккаунт на клиента; (3) встроенный провайдер `nous` в hermes-agent — **OAuth-only**: env `NOUS_API_KEY` объявлен в метаданных плагина, но runtime его не читает (кред — только `~/.hermes/auth.json`); refresh token **single-use и ротируется** — клонировать auth.json на флот нельзя, один логин = один живой агент.
- **Headless-логин:** device-code flow `hermes auth add nous --type oauth` → печатает URL `portal.nousresearch.com/manage-subscription?user_code=XXXX-XXXX` — одно человеческое подтверждение на инсталляцию (клиент может нажать со своего телефона — ложится на Telegram-флоу). Известный баг: страницу верификации иногда блокирует Vercel WAF (issue #12275, фикс PR #12281 в работе).
