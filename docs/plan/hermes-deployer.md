...
- [~] Один реальный деплой на боевом Hostinger-токене — **чекпойнт начат 2026-07-05**: `DRY_RUN=false` включён на проде, живой прогон из Telegram дошёл до реального `purchaseVM` (Hostinger списывал $12.39 с карты оператора). Платёж отклонён банком (нет средств) → воркер корректно: `failed` без ретрая покупки, без orphan-VM, Mini App показала ошибку. Fail-path проверен боем; happy-path ждёт пополнения карты и повторного прогона
- [ ] Ревью перед Phase 4
...
- [ ] Закрытый тест на реальном деплое (`DRY_RUN=false`) — под чекпойнтом с человеком
