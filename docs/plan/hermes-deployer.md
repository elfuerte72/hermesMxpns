...
**Acceptance criteria:**
- [ ] `apps/backend` (NestJS) собирается и стартует на :3000
- [ ] `apps/frontend` (Vite + React + TS) собирается и отдаёт index.html
- [ ] `packages/shared` экспортирует хотя бы один тип и реэкспортится из обоих приложений
- [ ] `npm run build` и `npm run lint` зелёные на корне
...
**Acceptance criteria:**
- [ ] `prisma migrate dev` накатывает схему без ошибок
- [ ] Сгенерированы TS-типы Prisma
- [ ] `docker compose up` поднимает Postgres + Redis
- [ ] Модели: `User(telegram_id, username)`, `Deploy(user_id, agent, bot_token_enc, bot_username, llm_provider, llm_key_enc, hostinger_vm_id, hostinger_script_id, status, bootstrap_token_hash, paid_until, timestamps)`, `ProvisioningLog(deploy_id, step, status, message, ts)`
...
**Acceptance criteria:**
- [ ] `encrypt(plaintext)` → `decrypt(ciphertext)` round-trip совпадает
- [ ] Разные шифртексты для одинаковых плейнтекстов (рандомный IV)
- [ ] Приложение падает с понятной ошибкой при отсутствии обязательного env
- [ ] Юнит-тесты на encrypt/decrypt/replay
...
**Acceptance criteria:**
- [ ] SDK установлен, типизированный клиент инстанцируется из `HOSTINGER_API_TOKEN`
- [ ] Методы-обёртки: `listTemplates()`, `listDataCenters()`, `getCatalog()`, `createPostInstallScript(name, content)`, `deletePostInstallScript(id)`, `purchaseVM(setup)`, `getVM(id)`, `listActions(vmId)`, `deleteVM(id)`
- [ ] Моки для тестов; один интеграционный тест (опционально, против боевого токена — за флагом)
...
- [ ] `npm run build` зелёный на корне
- [ ] Миграции накатываются, Postgres/Redis поднимаются
- [ ] Шифрование работает (тесты)
- [ ] Hostinger-клиент собирается (мок-тесты)
- [ ] Ревью с человеком перед Phase 2
...
**Acceptance criteria:**
- [ ] `POST /auth/validate-init` принимает `initData`, проверяет подпись, возвращает `{ user }`
- [ ] Невалидная подпись → 401
- [ ] Юзер upsert'ится в БД по `telegram_id`
- [ ] Юнит-тесты на валидацию подписи (валидный/невалидный/expired)
...
**Acceptance criteria:**
- [ ] Бот стартует с `BOT_TOKEN`, отвечает на `/start`
- [ ] Кнопка открывает `MINI_APP_URL`
- [ ] Длинные polling/Webhook — выбирается через env (dev: polling)
...
**Acceptance criteria:**
- [ ] `GET /llm-providers` возвращает массив провайдеров с полями `{ id, name, base_url, key_env, default_model, docs_url }`
- [ ] Каталог — кодом/конфигом, не в БД (статичный справочник)
- [ ] Юнит-тесты
...
**Acceptance criteria:**
- [ ] Валидный токен → 200 `{ username, id }`
- [ ] Невалидный токен → 422
- [ ] Токен уже используется другим активным деплоем → 409
- [ ] Тесты с моком Telegram API
...
- [ ] Бот открывает Mini App, TMA-авторизация работает
- [ ] Форма видит каталог провайдеров
- [ ] Можно валидировать bot token
- [ ] Ревью перед Phase 3
...
**Acceptance criteria:**
- [ ] `POST /deploys` → 202 `{ deploy_id }` (требует auth)
- [ ] Секреты шифруются перед записью; в БД только `_enc`
- [ ] Bootstrap-токен генерируется, в БД хранится только hash
- [ ] Job `deploy` ставится в очередь
- [ ] Тесты (мок очереди)
...
**Acceptance criteria:**
- [ ] Сгенерированный скрипт ≤ 48KB
- [ ] В скрипте нет bot token / LLM-ключа (только bootstrap URL с одноразовым токеном)
- [ ] Скрипт пишет логи в `/post_install.log`, корректно экранирует значения
- [ ] Юнит-тесты на содержимое скрипта (assert: нет секретов, есть нужные команды)
...
**Acceptance criteria:**
- [ ] Валидный токен + совпадающий IP → 200 `{ bot_token, llm_provider, llm_key, config_yaml }`
- [ ] Невалидный/重复 token → 404; IP-несовпадение → 403
- [ ] Токен горит после первого успешного использования (replay-защита)
- [ ] Тесты на все ветки
...
**Acceptance criteria:**
- [ ] Воркер проходит стейт-машину `pending → creating → configuring → ready/failed`
- [ ] Каждый шаг пишется в `ProvisioningLog`
- [ ] При ошибке — `deletePostInstallScript` + `deleteVM` (если создан) + уведомление
- [ ] Идемпотентность по `deploy_id` (не запустит дважды)
- [ ] Тесты с замоканным Hostinger SDK
...
**Acceptance criteria:**
- [ ] Валидный webhook → `status=ready`, сообщение клиенту
- [ ] Невалидная подпись/несуществующий deploy → 404
- [ ] Повторный webhook для уже ready → идемпотентно 200
- [ ] Тесты
...
- [ ] Полный flow на моках: TMA-форма → POST /deploys → воркер → webhook → ready
- [ ] Один реальный деплой на боевом Hostinger-токене (зафиксировать артефакты: созданный script, VM, лог `/post_install.log`)
- [ ] Ревью перед Phase 4
